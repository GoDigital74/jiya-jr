...
import { usePathname, useRouter } from "next/navigation";
import { urlForImage } from "@/sanity/lib/image";
// 👇 Added ChevronDown to the Lucide imports
...
export default function Header() {
  ...
  // 👇 FIX: Added a "dropdown" array to the Collection link
  const navLinks = [
    { name: "Home", href: "/" },
    { name: "About Us", href: "/about" },
    { 
      ...
      href: "/shop",
      dropdown: [
        { name: "School Awards", href: "/shop?category=School" },
        { name: "Corporate Gifts", href: "/shop?category=Business" },
        { name: "Sports Trophies", href: "/shop?category=Sports+Award" },
        { name: "Regular Mementos", href: "/shop?category=Regular" },
        { name: "Premium Gifts", href: "/shop?category=Gifts" },
      ]
    },
    { name: "Blog", href: "/blog" },
    { name: "Contact us", href: "/contact" },
    ...
    <>
      <header className="fixed top-0 w-full z-50 bg-white border-b border-gray-200 shadow-sm py-3">
        <div className="container mx-auto px-6 md:px-12 xl:px-20 flex justify-between items-center relative">
          ...
          {/* 👇 FIX: Updated Navigation mapping to support dropdowns */}
          <nav className="hidden lg:flex items-center justify-center gap-6 xl:gap-8 absolute left-[45%] top-1/2 -translate-x-1/2 -translate-y-1/2 w-max">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group">
                ...
                {/* Dropdown Container */}
                {link.dropdown && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 w-56 bg-white border border-gray-100 shadow-xl rounded-xl opacity-0 invisible translate-y-3 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-300 z-50 overflow-hidden">
                    <div className="py-2">
